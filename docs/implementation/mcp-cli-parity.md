...
non-compact results likewise use one block per requested target; target-local
state/recovery and global warnings; the separate ranked hit list; and at most one final
`Next:` action.
