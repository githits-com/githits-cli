...
Completed empty search uses the model's applicable action: generic query pivots are suppressed for evidence-limited or unsearched sources, indexing/provisional evidence prefers waiting or an indexed alternative, standalone site searches expose only a shorter/broader site query, and filter removal or symbol/code-grep pivots appear only when applicable. A completed result with both an evidence notice and `searchRef` emits one status continuation after the generic `Evidence may change.` trust statement. Terminal `DEFERRED`, `FAILED`, and `TIMEOUT` preserve disclosed evidence but prohibit further status calls; unknown statuses preserve the raw value and use the same conservative no-polling boundary. Promoted lifecycle/freshness warning prose and opaque evidence text remain in JSON but are not repeated in default MCP text; parser/query and structured constraint facts appear once below the outcome.
