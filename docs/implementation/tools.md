...
**Documentation corpora.** DOCS `sourceStatus` rows retain bounded physical `contributors` even when otherwise healthy. Repository contributors expose normalized `repositoryUrl`, full `commitSha`, freshness, and current-page `resultCount`; docpacks expose stable `siteKey` and selected published coverage. The JSON projection preserves meaningful zero/null values and every selected coverage field, but omits duplicate pair-level count/coverage and incidental healthy resolution metadata. Text-v1 renders one compact line per corpus and distinguishes `searched` from every `not searched` state; it reports hit counts only for searched corpora. Partial/capped coverage is published evidence, not a progress or retry signal, and does not create a warning when the contributor section already explains it.

`evidenceNotice` is carried once on initial and stored result envelopes and rendered once in text. A present `searchRef` supplies the `search_status` follow-up; without one, the notice is the only retry-variability guidance. `search_status(includeResults: true)` uses the same result projection and formatter—contributors are never copied onto generic progress targets, and `allowPartialResults` retains its separate pair-omission meaning.
