...
**`FILE_NOT_FOUND` vs `NOT_FOUND`**: `code_read` / `code_grep` can hit "path doesn't resolve" errors when an exact path scope is invalid. Exact-path grep misses carry backend `extensions.code: "FILE_NOT_FOUND"` plus `file_path`; the client preserves the backend message and maps the structured path into a surface-owned `details.action`. MCP names `code_files`, `path_prefix`, and `code_grep`; CLI JSON names `githits code files`, a path-prefix positional, and `githits code grep --path`. CLI terminal output names `code files`. `code_read` still supports generic `NOT_FOUND` from older/backend paths, and its structured recovery is likewise rendered with MCP or CLI-native names without classifying unrelated target misses as file errors.
