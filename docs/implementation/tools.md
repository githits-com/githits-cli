...
Empty grep adds scanned/in-scope counts, served target/ref context when known, and `Do not repeat this grep unchanged.` When the content index prunes candidates before verification, unequal counts explicitly identify the smaller value as `content-scanned after index pruning`, so it cannot be mistaken for an incomplete whole-target scan. Zero in-scope files direct the caller to loosen selectors; a nonzero scope directs it to change the pattern/check casing or switch to conceptual `search`. The same decision text is shared with CLI terminal stderr while plain CLI stdout remains grep-compatible and empty.
