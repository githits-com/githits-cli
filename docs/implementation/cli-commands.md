...
| `init uninstall` | — | `-y, --yes`, `--project`, `--keep-guidance` | Remove GitHits MCP server configuration from coding agents or supported project-local MCP files; optionally keep GitHits guidance files |
...
githits init uninstall --yes --keep-guidance      # Remove MCP config but retain GitHits guidance
```
