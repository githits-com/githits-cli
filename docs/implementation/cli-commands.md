...
| `read <target> [path]` | docs target/page ID, compact `target#symbol`, or package/repo target with exact path or selector | `--selector`, `--lines`, `--wait`, `--verbose`, `--json`; code also accepts `--start`, `--end`, `--repo-url`, `--git-ref` | Compact unified read; a compact package/repository fragment selects code with optional exact-path narrowing, while HTTP(S) URL fragments and repository documentation page IDs remain docs. An exact path selects code, and `--selector` selects a docs heading or indexed code symbol. The compact path calls `ReadService`/`Query.read` once. `--repo-url` remains the legacy compatibility path without selector. See [unified read](unified-read.md). |
