...
Global setup supports Claude Code, Cursor, Windsurf, VS Code / Copilot, Cline, Claude Desktop, Codex CLI, Pi, Gemini CLI, Google Antigravity, OpenCode, Hermes Agent, Zed, Junie, Qwen Code, Kiro, Kilo Code, Factory Droid, and Amazon Q CLI. It uses CLI MCP commands (Claude Code, Codex, and Gemini CLI), Amazon Q CLI commands, Pi adapter setup plus Pi-owned MCP config writes (Pi), and atomic config file writes (Cursor, Windsurf, VS Code, Cline, Claude Desktop, Google Antigravity, OpenCode, Hermes Agent, Zed, Junie, Qwen Code, Kiro, Kilo Code, Factory Droid). Claude setup removes legacy GitHits plugin and marketplace state, while Gemini setup removes legacy extension state; both then install the user-scoped stdio entry. CLI agents use read-only checks to determine global configuration status before prompting. Claude Code and Codex checks target only the `githits` server and run from a temporary directory so a project-local MCP entry cannot shadow user configuration. Existing non-canonical Claude entries remain installable and removable. Codex preserves every enabled existing `githits` entry, including custom or pinned commands. A disabled Codex entry is never overwritten by setup: init reports an actionable failure until the user re-enables or removes it, while uninstall can remove it. Probe failures remain distinct from confirmed missing or non-canonical entries during setup and verification.
...
**Complete-by-default results.** The CLI sends `allowPartialResults: false` unless `--allow-partial` is passed. If required indexing, crawling, or refresh work does not complete within the wait window, an active response returns a `searchRef` and progress summary. Stale-but-serveable evidence can accompany the reference while background refresh continues; callers use `search-status` rather than repeating `search` only while status is a known active value: `PENDING`, `INDEXING`, or `SEARCHING`. With `--allow-partial`, evidence from other ready target/source pairs can also be included while remaining work continues. Terminal `DEFERRED` retains any disclosed evidence and exact progress but stops advancing the `searchRef`; use that evidence now and start a new search later for a fresher snapshot. Future backend status values remain readable rather than failing response validation. The CLI prints the raw unrecognized status and preserves any evidence, but does not infer active or terminal semantics, claim indexing or no results, or poll the same reference; start a later new search instead. A missing or ambiguous standalone site can instead return terminal recovery guidance without a `searchRef`; callers retry an explicit `suggestedSiteTargets` label when present. `--limit` defaults to 10 results. `--wait` is in seconds (0-60, default 20).
