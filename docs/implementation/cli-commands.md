...
Repository code/docs now display enclosing declaration metadata and exact numbered
focused source. Scope rows carry full declaration ranges; the header carries the
read target/path and match range, so no redundant read command is printed. Source
is not prose-wrapped or renumbered. Match gutters remain visible without ANSI;
truncation and unavailable source are explicit. JSON additionally preserves
`repositoryEvidence` and `contentSafety`, including nullable branches, and derives
its bounded `followUp` from the backend's exact attributed `preferredRead` when
available. See [repository search evidence](tools.md) for the full
contract.
