...
| `code diff <target> <from>..<to>` *(experimental; config-gated)* | unversioned package/repository target and exact range, or `--repo-url` and range | `--patch`, `--stat`, `--name-only`, `--name-status`, `--max-files`, `--max-patch-bytes`, `--verbose`, `--json`, one glob after `--` | Silently dogfood bounded repository-wide tree diffs resolved from package versions or repository refs; local-only MCP `code_diff` is available when experimental tools are enabled, while public/remote MCP and shared Agent Skill guidance remain unchanged and enabled local MCP receives experimental guidance |
...
**Complete-by-default results.** The CLI sends `allowPartialResults: false` unless `--allow-partial` is passed. If required indexing, crawling, or refresh work does not complete within the wait window, the response returns a `searchRef` and progress summary. Stale-but-serveable evidence can accompany the reference while background refresh continues; callers use `search-status` rather than repeating `search`. With `--allow-partial`, evidence from other ready target/source pairs can also be included while remaining work continues. A missing or ambiguous standalone site can instead return terminal recovery guidance without a `searchRef`; callers retry an explicit `suggestedSiteTargets` label when present. `--limit` defaults to 10 results. `--wait` is in seconds (0-60, default 20).
