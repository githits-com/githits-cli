...
|---|---|---|---|
| **OAuth JWT** | `githits login` | REST example search (`/search`) | Interactive development |
| **API token** (`ghi-*`) | `GITHITS_API_TOKEN` env var | REST example search (`/search`) | CI, automation, quick setup |
...
- **Shared retry helper** — GitHits REST calls and package/source service calls both use the same token-refresh/retry flow, so auth drift is handled consistently across both service families. REST HTTP 401 and package/source GraphQL `AUTHENTICATION_REQUIRED` or `UNAUTHORIZED` responses enter this flow; `FORBIDDEN` stays an access-denied error. A backend `retryable: false` does not disable the one credential-refresh attempt, and a second authentication failure is returned without another refresh.
...
- **Terminal refresh failures** — An OAuth refresh endpoint's 4xx `invalid_grant` response rejects the refresh credentials regardless of whether its optional description is absent, unfamiliar, or mentions a client mismatch (RFC 6749 section 5.2). The CLI reloads storage to recover credentials replaced during the request, then conditionally clears only unchanged rejected tokens from the active backend. Grant rejection preserves the client registration. Invalid-client failures such as `invalid_client` or deleted client registrations additionally clear that registration so the next login performs fresh dynamic client registration. Description phrases alone do not establish grant rejection.
- **Supabase refresh responses** — The same terminal handling recognizes `refresh_token_not_found`, `refresh_token_already_used`, `session_not_found`, and `session_expired`. The parser accepts OAuth `error`, Supabase's legacy `error_code`/`msg` envelope (whose numeric `code` is an HTTP status), and its versioned string `code`/`message` envelope. Unknown codes, rate limits, and refresh conflicts do not become terminal grant failures. These formats are defined in Supabase Auth's [refresh handler](https://github.com/supabase/auth/blob/0907af9bd6be3c76f472c40a7dcc0dc34abeffaf/internal/api/oauthserver/handlers.go#L487), [token service](https://github.com/supabase/auth/blob/0907af9bd6be3c76f472c40a7dcc0dc34abeffaf/internal/tokens/service.go#L195), and [HTTP error serializer](https://github.com/supabase/auth/blob/0907af9bd6be3c76f472c40a7dcc0dc34abeffaf/internal/api/errors.go#L151).
- **Transient refresh failures** — Transport, timeout, and 5xx failures never clear refresh credentials. If the access token is expired, the current authenticated call surfaces the underlying refresh failure instead of misreporting that no local token exists, while leaving the expired candidate cached and persisted so the next `getToken()` call attempts refresh again. Status/token probes retain their non-throwing expired-auth behavior. Interactive init deliberately uses that non-throwing mode for its initial stored-token probe so an expired refresh failure can proceed to normal browser login instead of looping before OAuth. A later successful refresh persists token rotation normally; the expired access token is never served.
- **Missing client registration** — Refresh requires the stored dynamic OAuth client registration as well as the refresh token. If tokens exist but that companion keychain entry is missing or unreadable, authenticated calls return an explicit re-login error instead of the generic missing-token message. Interactive init treats this result as no usable token and proceeds to normal browser login, which can register a fresh client.
...
- **At init** (`src/commands/init/init.ts`) — Resolves auth through `createContainer()` at the login step so successful stored-token refresh still avoids browser login. Only interactive init requests the container's non-throwing refresh mode: an expired unclassified/transient refresh failure or missing/unreadable companion client registration yields `hasValidToken=false`, then the ordinary race-safe `loginFlow()` opens OAuth. Staged `--install-agents` and every default container caller retain throwing refresh semantics. When the init probe yields no token, the container clears only the expired session's non-secret auto-login metadata; retained OAuth credentials remain stored until successful login atomically replaces the session.
...
Uninstall intentionally leaves authentication untouched. To clear tokens manually after uninstall, use `npx githits@latest logout`. This removes stored tokens for the current MCP URL without server-side revocation (tokens expire naturally).
...
`LockedAuthStorage` serializes mutations and file-mode migration loads across CLI processes and long-running MCP servers with a per-user `~/.githits/auth.lock` directory. File-mode loads participate because migration can reconcile ambiguous plaintext candidates by writing the canonical record and clearing legacy copies; ordinary keychain loads remain read-only and avoid this lock. The lock records PID and process start time so definitely dead owners and reused PIDs can be reclaimed without stealing live locks. `ESRCH` is the only process-probe result treated as definite absence; permission and unexpected probe failures retain the lock. A missing or failed process-start lookup is also unknown evidence and retains the lock. PowerShell and `ps` identity probes are capped at five seconds. Unreadable or malformed owner metadata retains the lock, while a genuinely missing owner file permits age-bounded recovery when the directory is empty or contains only valid reclaim claims. Contenders retry normal lock acquisition every 25 ms but throttle the heavier stale-owner identity check to once per second. Each operation retains its exact acquired owner identity through release, so overlapping operations on one storage instance cannot release each other's locks. On release, owner metadata is read up to three times when temporarily unavailable, and transient Windows sharing violations while removing cleanup files are retried. After removing its verified owner record, the releaser atomically renames the now-empty directory to an owner-hashed sibling before removing it. Successors therefore create `auth.lock` without overlapping removal of that same Windows pathname, avoiding the Windows/Bun handoff where `mkdir(auth.lock)` can return `EPERM` while `rmdir(auth.lock)` completes. Token refresh holds this lock while it reloads stored credentials, calls the token endpoint, and saves or clears the result.

When an owner is proven dead, contenders serialize reclamation with an exclusive file whose name contains a SHA-256 hash of that owner ID. The claim holder re-reads `owner.json`, removes it only if the owner ID still matches, releases its exact claim with bounded sharing-violation retries, and removes only an empty lock directory. A delayed reclaimer that no longer sees its exact owner removes only its own claim, never the directory. This prevents dead-owner reclaimers from deleting a successor lock or a directory being released by its real owner. An abandoned valid claim in an old ownerless directory is swept before empty-directory removal; a matching owner-scoped claim abandoned beside its retained dead-owner metadata still requires manual recovery. A release-time owner read or deletion failure retains the lock while the recorded process lives, then becomes reclaimable after that process exits if its metadata can be read. A failed normal-release rename leaves an ownerless `auth.lock` eligible for the existing age-bounded recovery; a crash or cleanup failure after a successful rename can leave an inert `auth.lock.release-<owner-hash>` sibling, potentially containing a reclaim claim, that does not block future work. Persistently malformed or unreadable owner metadata, unknown ownerless entries, matching owner-scoped claims abandoned beside their retained dead-owner metadata, cleanup files that cannot be removed after bounded retries, or contested lock directories that cannot be removed during stale recovery remain fail-closed and surface the existing timeout with manual-removal guidance. Stop all GitHits CLI and MCP processes before removing the reported `auth.lock` directory manually. Because older processes do not honor reclaim claims or the rename-away release handoff, restart long-running local MCP processes after upgrading the CLI so every local auth consumer uses the hardened protocol.
       ...
       └─ no env token → create TokenManager + RefreshingGitHitsService
            └─ TokenManager.getToken() for initial check
                 ...
                 ├─ stored token expired? → reactive refresh
                  ...
                  │    ├─ transient failure → reload storage; retain unchanged refresh credentials; throw by default or return no token for the interactive init probe
...
- **Token refresh fails during init** — The token manager first reloads storage in case another process refreshed credentials. Interactive init treats an unclassified/transient expired-token refresh failure as no usable token and starts the normal browser sign-in flow; choosing retry performs another complete sign-in attempt instead of repeating only the pre-login refresh. Transient failures retain unchanged refresh credentials, and only classified terminal failures clear stale state from the active backend.
...
- **System keychain unavailable** — In default keychain mode, reads and writes fail with storage guidance rather than treating credentials as missing or writing plaintext credentials. Use `GITHITS_API_TOKEN`, fix/unlock the keychain, or explicitly configure `auth.storage = "file"` if plaintext local storage is acceptable.
