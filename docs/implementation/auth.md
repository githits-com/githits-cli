...
- **System keychain unavailable** — In default keychain mode, reads, writes, and deletes fail with storage guidance rather than treating credentials as missing or writing plaintext credentials. Use `GITHITS_API_TOKEN`, fix/unlock the keychain, or explicitly configure `auth.storage = "file"` if plaintext local storage is acceptable.
