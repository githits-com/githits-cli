...
const DESCRIPTION =
  ...
  "Complete by default — if required indexing or refresh outlasts the wait window, the response carries a `searchRef`; do not repeat `search`, pass that reference to `search_status`. Stale-but-serveable or provisional-but-queryable evidence can accompany the reference while indexing or refresh continues. A missing or ambiguous site can instead return terminal recovery guidance without a `searchRef`; follow any `suggestedSiteTargets` explicitly rather than calling `search_status`. " +
