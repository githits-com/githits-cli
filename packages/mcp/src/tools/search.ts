...
const DESCRIPTION =
  ...
  "After discovery, use `code_grep` for deterministic exact-pattern occurrences. Follow each hit's generated `followUp`. From text, pass a `[docs page]` target unchanged to `docs_read` (a fragment needs no bounds); use repo-doc targets/ranges with `docs_read`, and repo code/symbol targets, paths, and ranges with `code_read`." +
  `\n\n${SEARCH_GUARDRAIL}`;
