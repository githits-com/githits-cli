...
const DESCRIPTION =
  ...
  "Complete by default — active `PENDING`, `INDEXING`, or `SEARCHING` progress carries a `searchRef`; do not repeat `search`, pass that active reference to `search_status`. Stale-but-serveable evidence can accompany the reference while refresh continues. Terminal `DEFERRED` preserves disclosed evidence but stops that reference; use the evidence now and run a new search later for a fresher snapshot. A missing or ambiguous site can instead return terminal recovery guidance without a `searchRef`; follow any `suggestedSiteTargets` explicitly rather than calling `search_status`. " +
