...
- For an exact standalone documentation site, call `search` with target `site:<host[/path]>` and source `docs`. Follow a returned `searchRef` with `search_status` only while progress is `PENDING`, `INDEXING`, or `SEARCHING`. For `DEFERRED` or an unrecognized status, keep any disclosed evidence, do not poll the reference again, and start a new search later. If a missing or ambiguous site instead returns `suggestedSiteTargets`, retry one exact suggestion explicitly rather than rewriting the target or treating it as an alias.
