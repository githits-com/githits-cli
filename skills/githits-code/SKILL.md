...
- Follow rendered continuation/recovery actions. Use `search-status <searchRef>` only when search explicitly supplies that follow-up; never repeat search to poll or poll a stopped reference. Read the search reference below for status/wait details.
