...
- If search returns a `searchRef` with active `PENDING`, `INDEXING`, or `SEARCHING` progress, continue with `githits search-status <searchRef>` instead of repeating the original search. Its bounded wait defaults to 20 seconds; use `--wait <seconds>` with an integer from 0 to 60 to adjust it. For `DEFERRED` or an unrecognized status, preserve any disclosed evidence, do not poll the reference again, and start a new search later.
